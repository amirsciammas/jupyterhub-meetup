...
There are 2 types of deployment : 
  ...
  - local - Creates and uses local mongodb with default users to authemnticate the hub
  
  - production - uses existing mongodb
...
Clone the repo and CD into it 
    ...
    cd <pathforprojects>
    git clone https://bitbucket.org/citydimension/jupyterhub.git
    cd jupyterhub 
...
For example 
    ...
    OAUTH_CLIENT_SECRET=<yoursecert>
  
    OAUTH_CALLBACK_URL=https://jupyter.dimension.ws/hub/oauth_callback

    OAUTH2_TOKEN_URL=https://oauth.dimension.ws/connect/token
    OAUTH2_AUTHORIZE_URL=https://oauth.dimension.ws/connect/authorize
    OAUTH2_USERDATA_URL=https://oauth.dimension.ws/connect/userinfo

    OAUTH2_USERNAME_KEY=preferred_username



MongoDB Auth (For local or dev deployment)
------------------------------------------

Set MongoDB credentials in .env file :

    vi .env

Fro 'production' deployment use the actual credentials of the database
...
For 'local' deployment set the MongoDB server details and default credentials : 

    # Will be used when creating the local mongodb instance
    MONGO_INITDB_ROOT_USERNAME=mongoAdmin 
    MONGO_INITDB_ROOT_PASSWORD=pwd
    MONGO_INITDB_DATABASE=jupyterhub
    
    # Credentials for mongodb database
    LOCAL_MONGO_HOST=jupyterhub-mongo
    LOCAL_MONGO_USER=applic
    LOCAL_MONGO_PASSWORD=pwd
    LOCAL_MONGO_DB=jupyterhub

* note : those values corosponde to the values from mongo-entrypoint/init_script.sh, if you change on of those make sure you update the other

Open config file (jupyterhub_config.py) , comment out the below : 
...
Open config file (jupyterhub_config.py) , uncomment the below : 

    #c.JupyterHub.authenticator_class = 'mongo_authenticator.RemoteMongoAuthenticator'


Set Hub Administrator user in userlist file :
...
Prepare single user notebooks

    make notebook_image
    
Run some checks and build images 
...
Tear down 
    ...
    # For 'local'
    docker-compose -f docker-compose.localmongo.yml down


How to access JupyterLab ? 

Login to your Jupyter notebook, and replace this section in the url 'tree?' with 'lab'

for example : 

    Notebook : /user/myusername/tree?
    JupyterLab :/user/myusername/lab


To see logs execute the below from the repo directory 
...
- Notebook 5.2
- JupyterLab 0.31.8
- Single user image "jupyter/all-spark-notebook" (see full info: https://github.com/jupyter/docker-stacks/tree/master/all-spark-notebook)
- Admin access to other users 
- 'local' includes a local MongoDB 
- MongoDB Authentication (Uses mongodb to store user/pass information and authenticate aginst it)
- No SSL configuration , assumes NGINX in place (https://amirsciammas@bitbucket.org/citydimension/nginx-docker.git)
- Dimension logo
...
- Add steps for other actions like : update configs, add users , change images etc.
- Add image form (https://github.com/jupyterhub/wrapspawner)
- Move MongoDB authemtication to it's own repo 
- Add NGINX to 'local' with/out SSL 
- Add 'production' option for SSL (in case there is no NGINX)




## Below is the original README , which include a lot of information on the repo

* This repo is a fork from : https://github.com/jupyterhub/jupyterhub-deploy-docker
